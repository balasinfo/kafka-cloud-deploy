...
curl -L --connect-timeout 5 \
    ...
    --retry-delay 3 \
    --retry-max-time 140 \
    -o $base_name.tgz ${repo}/${version}/$base_name.tgz
#-L ${repo}/${version}/$base_name.tgz -o $base_name.tgz
...
cat config/server.properties \
    ...
    | sed 's|advertised.listeners=PLAINTEXT://your.host.name:9092|advertised.listeners=PLAINTEXT://broker.kafka.cgcloud.eu|' \
