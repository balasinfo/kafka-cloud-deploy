...
module "bastion_server" {
  ...
  ami                          = data.aws_ami.image_latest.image_id
...
module "kafka" {
  ...
  key_name                = module.ssh_key_pair.key_name
  zookeeper_ami           = data.aws_ami.image_latest.image_id
  ...
  kafka_ami               = data.aws_ami.image_latest.image_id
  kafka_user              = var.user
  ...
  kafka_broker_alb_arn    = module.kafka_broker_alb.default_target_group_arn
}

module kafka_broker_alb {
...
module kafka_route53_cluster_hostname {
  ...
  zone_id = var.route53_zone_public_id
  records = [module.kafka_broker_alb.alb_dns_name]
}
