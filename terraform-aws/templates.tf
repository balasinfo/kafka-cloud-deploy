 ...
 */
data template_file "zookeeper-init" {
  count = local.zk_count
  template = file("${path.module}/../templates/cloud-init/zookeeper-init.yml")
  vars = {
    service = "zookeeper"
    metric = "ZookeeperStatus"
    count = local.zk_count
    id = count.index+1
#    ip_addrs = join(" ", .*.private_ip)
  }
}

data template_file "kafka-init" {
  count = local.zk_count
  template = file("${path.module}/../templates/cloud-init/kafka-init.yml")
  vars = {
    service = "kafka"
    metric = "KafkaStatus"
...
data template_file "zookeeper-state-change" {
  template = file("${path.module}/../templates/event_patterns/ec2-state-change.json")
  vars = {
